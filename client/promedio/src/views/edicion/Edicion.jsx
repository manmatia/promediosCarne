...
function Edicion() {
  ...
  useEffect(() => {
    if (cortes.length > 0 && selectedkgMedia) {
      ...
      cortes.forEach(corte => {
        ...
        const nuevoPrecioValue = nuevoPrecio[corte.id] || precioVenta;
        totalNuevoPrecioValue += nuevoPrecioValue * corte.kilos;
      });
  ...
  const generateWorksheetData = () => {
    ...
    const rowHeights = new Array(worksheetData.length).fill({ hpx: 20 });
    worksheet['!rows'] = rowHeights;
  ...
  const exportToXml = () => {
    const xmlData = generateXmlData(); // Función para generar el XML
  ...
  const generateXmlData = () => {
    // Aquí generas los datos en formato XML basado en tus cortes y precios
    let xmlData = '<?xml version="1.0" encoding="UTF-8"?>\n<productos>\n';

    cortes.forEach(corte => {
      xmlData += `<producto>\n`;
      xmlData += `<nombre>${corte.corte}</nombre>\n`;
      xmlData += `<precio>${(nuevoPrecio[corte.id] || parseFloat(corte.precio_venta)).toFixed(2)}</precio>\n`;
      xmlData += `</producto>\n`;
    });
  ...
  const shareXml = async () => {
    ...
    const blob = new Blob([xmlData], { type: 'text/xml' });
    const file = new File([blob], 'productos.xml', { type: blob.type });
    ...
    if (navigator.share && navigator.canShare({ files: [file] })) {
      try {
        await navigator.share({
          files: [file],
          title: 'Archivo XML',
          text: 'Consulta los productos en XML.'
        });
        console.log('Archivo compartido exitosamente');
      } catch (error) {
        console.error('Error al compartir el archivo:', error);
      ...
      console.warn('La API de compartir archivos no es compatible con este navegador.');
      // Puedes implementar una alternativa aquí si `navigator.share` no está disponible
    }
  };
  ...
  return (
    <>
      ...
      <div className="mt-3 ml-2">
        <button className="btn btn-success" onClick={exportToExcel}>Descargar en Excel</button>
        <button className="btn btn-primary ms-2" onClick={shareXml}>Compartir</button>
      </div>
    </>
  );
}
